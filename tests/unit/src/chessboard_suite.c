...
int init_suite_chessboard(void) {
  ...
  /* Getting a board from a FEN string, here is the starting position fen string */
  fenToBoard(board2, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
  /*Just to show off */
  printf("\n\n");
  printBoard(board2);
  printf("\n\n");  

  /* Getting a board from another FEN string, here is the starting position + 1.e4 e5*/
  fenToBoard(board3, "rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1");
  /*Just to show off */
  printf("\n\n");
  printBoard(board3);
  printf("\n\n");  
...
void testNormalValues(void) {
  ...
  /*From the FEN string describing the starting position, we should have pieces correctly placed */
  CU_ASSERT_EQUAL(board2[0],'r');
  CU_ASSERT_EQUAL(board2[63],'R');

  /*From start pos + 1.e4 e5, we're checking the position of pawns that has moved */
